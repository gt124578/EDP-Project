...
plt.show()
L2_error = np.sqrt(np.sum((u - u_exact_mat)**2) * dx * dt)
print(f"Erreur en norme L2: {L2_error}")
L2_relative=np.linalg.norm(u_exact_mat)/np.linalg.norm(u)
...
plt.show()
L2_error = np.sqrt(np.sum((u - u_exact_mat)**2, axis=1) * dx)  # Somme sur l'espace
